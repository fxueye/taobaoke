	 ...
	 :hover-class="disabled === true || disabled === 'true' || showSwitch === true || showSwitch === 'true' ? '' : 'uni-list-cell--hover'" @click="onClick">
		<view class="uni-list-cell__container">
			...
			<view class="uni-list-cell__icon" v-else-if="showExtraIcon === true || showExtraIcon === 'true'">
				<uni-icon :fontClass="extraIcon.fontclass" :color="extraIcon.color" :size="extraIcon.size" :type="extraIcon.type"></uni-icon>
			</view>
			...
			<view class="uni-list-cell__extra" v-if="showBadge === true || showBadge === 'true' || showArrow === true || showArrow === 'true'||showSwitch === true || showSwitch === 'true' || showRightNote === 'true' | showRightNote === true">
				<view class="uni-list-right-note" v-if="showRightNote === true || showRightNote==='true'">{{rightNote}}</view>
...
<script>
	...
	export default {
		...
		props: {
			title: String, //列表标题
			note: String, //列表描述
			rightNote:String,//右侧提示文字
		
			disabled: { //是否禁用
				type: [Boolean, String],
				default: false
			},
			showRightNote:{ //是否显示右侧提示文字
				type: [Boolean, String],
				default: true
			},
			showArrow: { //是否显示箭头
